...
if adc_res != float('inf'):
    rpu_config.forward.inp_res = adc_res
...
plt.savefig(f'figures/{n_epochs}_epochs_4_layers_{neurons}_neurons_{fault_ratio}_fault_{slices}_slices_{batch_size_train}_batch_size_{type}_loss_sim.png')

# Save model
model_path = f"{n_epochs}_epochs_4_layers_{neurons}_neurons_{fault_ratio}_fault_{slices}_slices_{batch_size_train}_batch_size_{type}_loss2_analog.mdl"
torch.save(best_model.state_dict(), model_path)


# Save the loss trend
with open(f"figures/{n_epochs}_epochs_4_layers_{neurons}_neurons_{fault_ratio}_fault_{slices}_slices_{batch_size_train}_batch_size_{type}_loss_sim_data.pk","wb") as file:
