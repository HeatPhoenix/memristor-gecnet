...
# Save the loss trend
with open(f"figures/{n_epochs}_epochs_4_layers_{neurons}_neurons_{fault_ratio}_fault_{slices}_slices_{batch_size_train}_batch_size_{type}_drift_sim_{timestamp_str}_data.pk","wb") as file:
    pkl.dump((loss_list, loss_val_list), file)

# Write to the CSV
fields = [type_str, n_epochs, neurons, fault_ratio, slices, batch_size_train, learning_rate]

with open(fr'data/{neurons}_4_189_{n_epochs}_drift_{slices}_data.csv', 'a') as f:
    writer = csv.writer(f)
    for i in range(len(loss_list)):
        row = fields + [loss_list[i], loss_val_list[i]]
        writer.writerow(row)
